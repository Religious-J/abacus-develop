...
struct gemm_op
{
    ...
    ///
    /*
d: 指向设备的指针，表示在哪个计算设备上执行计算。
transa: 一个字符，表示是否对矩阵 A 进行转置。可能的值为 'N'（不转置）和 'T'（转置）。
transb: 一个字符，表示是否对矩阵 B 进行转置。可能的值为 'N'（不转置）和 'T'（转置）。
m: 矩阵乘法的第一维度（矩阵 C 的行数）。
n: 矩阵乘法的第二维度（矩阵 C 的列数）。
k: 矩阵乘法的第三维度（矩阵 A 的列数（不转置时）或行数（转置时））。
alpha: 标量乘数 α，用于标量与矩阵乘积的系数。
a: 指向输入矩阵 A 的指针。
lda: 矩阵 A 的主维度（即矩阵 A 的行数）。
b: 指向输入矩阵 B 的指针。
ldb: 矩阵 B 的主维度（即矩阵 B 的行数）。
beta: 标量乘数 β，用于标量与矩阵 C 的乘积的系数。
c: 指向输入和输出矩阵 C 的指针。
ldc: 矩阵 C 的主维度（即矩阵 C 的行数）。
    */
    /// Output Parameters
